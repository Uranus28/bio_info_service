...
import UserStore from "../../store/ServicesStore";
...
export const getResponse = async (
  ...
  function isYear(item: any) {
    if (item["id"] === Number(year)) {
      return item["name"];
    }
  }
  let year_n = Number(services.Years.find(isYear)!["name"]);
  ...
  if (typeSearch === TypeSearch.BooksByName) {
    ...
    response = await ItemService.getBooksByTheme(
      limit,
      page,
      services.Themes[themes - 1].name
    );
  } else if (typeSearch === TypeSearch.ArticleByTheme) {
    response = await ItemService.getArticlesByTheme(
      limit,
      page,
      services.Themes[themes - 1].name
    );
  } else if (typeSearch === TypeSearch.WebByTheme) {
    response = await ItemService.getSitesByTheme(
      limit,
      page,
      services.Themes[themes - 1].name
    );
  } else if (typeSearch === TypeSearch.AuthorByTheme) {
    response = await ItemService.getAuthorsByTheme(
      limit,
      page,
      services.Themes[themes - 1].name
    );
    ...
    response = await ItemService.getBooksByDatePublished(limit, page, year_n);
  } else if (typeSearch === TypeSearch.ArticleByDate) {
    response = await ItemService.getArticleByDatePublished(limit, page, year_n);
  } else if (typeSearch === TypeSearch.WebByDate) {
    response = await ItemService.getWebByDatePublished(limit, page, year_n);
  } else {
    response = await ItemService.getArticlesByTheme(
      limit,
      page,
...
export const getTypeSearchChangeDate = (resource: string) => {
  let typeSrch;
  switch (resource) {
    case "1":
...
export const getTypeSearchChangeActual = () => {
  return TypeSearch.ActualData;
};

export const getTypeSearchChangeName = (resource: string) => {
  let typeSrch;
  switch (resource) {
    case "1":
...
export const getTypeSearchChangeTheme = (resource: string) => {
  let typeSrch;
  switch (resource) {
    case "1":
...
export const cardsItems = (data: any[], typeSearch: number) =>
  data.map((item, index) => {
    if (
      typeSearch === TypeSearch.AuthorByName ||
      typeSearch === TypeSearch.AuthorByTheme
    ) {
      return <CardAuthor data={item} key={index}></CardAuthor>;
    } else {
      return <CardItem data={item} key={index}></CardItem>;
    }
  });
