...
//   updatePage: boolean;
// }
export const Concepts = ({ updatePage }) => {
  const [form] = Form.useForm();
  const [subAreas, setSubAreas] = useState([]);
  const [curSubjectArea, setCursubjectArea] = useState("");
  const [curTerms, setCurTerms] = useState([]);
  const [isVisibleCreateSubjectAreaForm, setIsVisibleCreateSubjectAreaForm] =
    useState(false);
  ...
  const onChangeTerm = (e) => {
    ...
    setFilterTerms(
      curTerms.filter(
        (term) =>
          term.termStr.toLowerCase().indexOf(e.target.value.toLowerCase()) != -1
      )
    );
  };
  ...
  if (isLoading) {
    ...
    return (
      <Form
        ...
        autoComplete="off"
      >
        <Form.Item label="Предметная область" style={{ margin: "10px auto" }}>
          <Select
            showSearch
            style={{ width: 200 }}
        ...
        <CreateTerm
          isVisible={isVisibleCreateTermForm}
          setIsVisible={setIsVisibleCreateTermForm}
          subjectArea={curSubjectArea}
          onUpdate={onUpdate}
        ></CreateTerm>
        <CreateSubjectArea
          isVisible={isVisibleCreateSubjectAreaForm}
          setIsVisible={setIsVisibleCreateSubjectAreaForm}
          onUpdate={onUpdate}
        ></CreateSubjectArea>
      </Form>
    );
